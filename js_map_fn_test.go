...
func TestEmitFunction(t *testing.T) {
	...
	rows, err := mapper.CallFunction(`{}`, "doc1", VbucketNo(0), VbucketSeq(0))
...
func testMap(t *testing.T, mapFn string, doc string) []*ViewRow {
	...
	rows, err := mapper.CallFunction(doc, "doc1", VbucketNo(0), VbucketSeq(0))
...
func TestEmptyJSMapFunction(t *testing.T) {
	...
	rows, err := mapper.CallFunction(`{"key": "k", "value": "v"}`, "doc1", VbucketNo(0), VbucketSeq(0))
	assertNoError(t, err, "CallFunction failed")
